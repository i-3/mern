...
export default function Record() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const response = await fetch(
        `http://${API_URL}/record/${params.id?.toString()}`
      );
      if (!response.ok) {
  ...
  async function onSubmit(e: any) {
    ...
    try {
      ...
      if (isNew) {
        ...
        response = await fetch(`http://${API_URL}/record`, {
          method: 'POST',
          headers: {
        ...
        response = await fetch(`http://${API_URL}/record/${params.id}`, {
          method: 'PATCH',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  return (
    <>
      ...
      >
        <div
          className='grid grid-cols-1 gap-x-8 gap-y-10 border-b
